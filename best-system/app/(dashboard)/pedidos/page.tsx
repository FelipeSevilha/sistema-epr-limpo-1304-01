...
export default function PedidosPage() {
  ...
  async function fetchPedidos() {
    try {
      ...
      const [pedidosRes, ordensRes, fichaRes, estoqueRes, clientesRes] = await Promise.all([
  ...
  function openEditForm(pedido: PedidoRow) {
    ...
    setClienteSearch(pedido.cliente_nome || '');
    setShowClienteList(false);
    setMargemAlvo(30);
    setFormOpen(true);
  }
  ...
  function selecionarCliente(cliente: ClienteRow) {
                    ...
                    {clientesFiltrados.map((cliente) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {(cliente.email || cliente.contato) && (
                            <div className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                              {[cliente.contato, cliente.email].filter(Boolean).join(' • ')}
                            </div>
                          )}
                        </button>
                      );
